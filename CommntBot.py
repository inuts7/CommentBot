...
| | (_| |  / __/ \ V /| | (_) | | | | |  __/ | |  __/  _| |
 \ \__,_| |_____| \_/_/ |\___/  |_| |_|\___|_|  \___| (_) |
  \____/            |__/                               /_/ 
''')
print('  |   Comment Bot by @2vj6 | inuts7#5742   |\n')
...
x_ig_app_id = str(randint(90000000000, 1000000000000000))
log_head = {
    'accept': '*/*',
...
def comment(post_link, id):
    ...
    head = {
        ...
        'x-csrftoken': csrftoken,
        'x-ig-app-id': x_ig_app_id,
        'x-ig-www-claim': 'hmac.AR3ihiFU4BpZNtr3itbgVqLCE4hOBhysg8trxAGmUhxZqvUH',
    ...
    if ask == 1:
        ...
        while 1:
            sleep(slp)
            data = {
                'comment_text': choice(lists),
                'replied_to_comment_id': ''
            }
            send = req.post(api, headers=head, data=data, cookies=cookies)
            if '"status":"ok"' in send.text:
                sent += 1
                print(f'\r- Sent : {sent} , spam : {spam} , baned : {baned}', end=' ')
            elif 'Please wait a few minutes before you try again.' in send.text:
                spam += 1
                print(f'\r- Sent : {sent} , spam : {spam} , baned : {baned}', end=' ')
                sleep(300)
            else:
        ...
        lists = []
        count = int(input('- How many words do u want to put in ? : '))
        for cum in range(count):
            print(f'- Now The {cum + 1} comment')
            coment = input('- input comment : ')
            lists.append(coment)
        clear()
        print('''
           ____    ____        _  __     _                     __  
...
def login():
    ...
    if 'userId' in log.text:
        ...
        if '?igshid' in post_link:
            ...
            info = f'{cut}?__a=1'
        elif 'ig_web_copy_link' in post_link:
            cut = post_link.split('?')[0]
            info = f'{cut}?__a=1'
        else:
            info = f'{post_link}?__a=1'
        ...
        print("- Check your password & try again !")
        close()
